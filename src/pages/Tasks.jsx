...
import { Plus, CheckCircle, Circle, Calendar, BookOpen, ChevronUp, Trash2, AlertTriangle, AlertCircle, CheckCircle2, Loader2, ChevronDown } from 'lucide-react';
import { supabase } from '../supabaseClient';

const Tasks = ({ theme, darkMode, assignments = [], courses = [], loading, refreshData }) => {
    ...
    return (
      <div style={{ padding: '20px', minHeight: '100vh', backgroundColor: theme?.bg, paddingBottom: '120px' }}>
        ...
        <div style={{ display: 'flex', gap: '8px', marginBottom: '20px' }}>
          {[1, 2, 3].map(i => <div key={i} className="skeleton" style={{ width: '70px', height: '32px', borderRadius: '20px' }} />)}
        </div>
        <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
          {[1, 2, 3, 4, 5].map(i => <div key={i} className="skeleton" style={{ height: '75px', borderRadius: '20px' }} />)}
        </div>
        <style>{`
          .skeleton { 
            ...
            {isDropdownOpen && (
              <div style={{ position: 'absolute', top: '100%', left: 0, right: 0, backgroundColor: theme?.card, borderRadius: '12px', border: `1px solid ${theme?.border}`, marginTop: '8px', zIndex: 100, maxHeight: '200px', overflowY: 'auto', boxShadow: '0 10px 25px rgba(0,0,0,0.2)' }}>
                {courses.length > 0 ? courses.map(course => (
                  ...
                  <div style={{ padding: '12px 16px', fontSize: '12px', opacity: 0.5 }}>No courses found. Add one in Course Manager.</div>
                )}
              </div>
            )}
          </div>
              ...
              <div key={task.id} style={{ padding: '18px', backgroundColor: theme?.card, borderRadius: '20px', border: `1px solid ${theme?.border}`, display: 'flex', alignItems: 'center', gap: '12px', opacity: task.status === 'completed' ? 0.6 : 1 }}>
                ...
                <div style={{ flex: 1 }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginTop: '4px' }}>
                    <span style={{ fontSize: '10px', color: courseInfo.color, fontWeight: '800', display: 'flex', alignItems: 'center', gap: '4px' }}>
                      <div style={{ width: '6px', height: '6px', borderRadius: '50%', backgroundColor: courseInfo.color }} />
                      {courseInfo.name}
                    </span>
